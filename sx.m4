...
__sx_str_isep() {
	...
	set -- "${1}" "${2-}" "${3-}" "$((${4:-1}))" "$((${5:-${SX_NUM_I32_MAX}}))" "$((${6:-0} & SX_STR_ISEP_CB))" "" "" 0 "" "${4:-1}"

	# インターバル分の '?' を生成
	SX_CFG_UNSET_SOFT=2 __sx_str_rep __sx_str_isep_qm_ '?' "${4#[+-]}"
	set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "" "${__sx_str_isep_qm_}" 0 "" "${11}"
	...
	if M_NUM_LT([|0|], [|${4}|]); then
		...
		do
			set -- "${@}" "${2#${8}}"
			set -- "${@}" "${2%"${12}"}"
			# $12: next_str, $13: chunk
			case "${6}" in
				0) set -- "${1}" "${12}" "${3}" "${4}" "$(( ${5} - 1 ))" "${6}" "${7}${13}${3}" "${8}" "$(( ${9} + 1 ))" "${10}${13}" "${11}";;
				*)
					set -- "${@}" "${10}${13}"
					"${3}" __sx_str_isep_cb_ "${11}" "${14}" "${12}" "$(( ${9} + 1 ))" || set -- "${@}" "${?}"
					set -- "${1}" "${12}" "${3}" "${4}" "$(( 0 < ${15-0} ? 0 : ${5} - 1 ))" "${6}" "${7}${13}${__sx_str_isep_cb_-}" "${8}" "$(( ${9} + 1 ))" "${14}" "${11}"
		...
		# Backward (逆方向)
		set -- "${1}" "${2}" "${3}" "$(( ${4} * -1 ))" "${5}" "${6}" "" "${8}" 0 "" "${11}"
		while
			M_NUM_LT([|${4}|], [|${#2}|]) && M_NUM_NE([|${5}|], [|0|])
		do
			set -- "${@}" "${2%${8}}"
			set -- "${@}" "${2#${12}}"
			# $12: prev_str, $13: chunk
			case "${6}" in
				0) set -- "${1}" "${12}" "${3}" "${4}" "$(( ${5} - 1 ))" "${6}" "${3}${13}${7}" "${8}" "$(( ${9} + 1 ))" "${13}${10}" "${11}";;
				*)
					set -- "${@}" "${13}${10}"
					"${3}" __sx_str_isep_cb_ "${11}" "${12}" "${14}" "$(( ${9} + 1 ))" || set -- "${@}" "${?}"
					set -- "${1}" "${12}" "${3}" "${4}" "$(( 0 < ${15-0} ? 0 : ${5} - 1 ))" "${6}" "${__sx_str_isep_cb_-}${13}${7}" "${8}" "$(( ${9} + 1 ))" "${14}" "${11}"
					unset __sx_str_isep_cb_
					;;
			esac
		done
