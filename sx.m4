...
changequote({{, }})dnl
define({{SX_STR_EQ2}}, {{case $1 in $2) :;; *) ! :;; esac}})dnl
	...
	for __sx_ex_remap_map_ in "${@}"; do
		...
		case "${1}" in
			...
			*-*)
				...
				;;
			!*)
				if ! SX_STR_EQ2({{"${1#!}"}}, {{"${__sx_ex_remap_sts_}"}}); then
					__sx_ex_remap_sts_="${2}"
					break
				fi
				;;
...
__sx_var_list_dep() {
	...
	while ! SX_STR_EQ2({{"${#}"}}, {{0}}); do
		...
		if __sx_var_is_arr "${1}"; then
			...
			while ! SX_STR_EQ2({{"${__sx_var_list_dep_i_}"}}, {{"${__sx_var_list_dep_len_}"}}); do
...
__sx_var_list_ro() {
	...
	IFS="${SX_STR_LF}" sx_util_eval '
		for __sx_var_list_ro_ln_ in $(readonly -p); do
			...
			if
				! SX_STR_EQ2({{"${__sx_var_list_ro_vn_}"}}, {{"${__sx_var_list_ro_ln_}"}}) &&
...
__sx_var_list_set() {
	...
	IFS="${SX_STR_LF}" sx_util_eval '
		for __sx_var_list_set_ln_ in ${__sx_var_list_set_set_}; do
			...
			if
				! SX_STR_EQ2({{"${__sx_var_list_set_vn_}"}}, {{"${__sx_var_list_set_ln_}"}}) &&
...
__sx_var_unset() {
	while ! SX_STR_EQ2({{"${#}"}}, {{0}}); do
		if __sx_var_is_arr "${1}"; then
			...
			__sx_var_unset_i_=0
			while ! SX_STR_EQ2({{"${__sx_var_unset_i_}"}}, {{"${__sx_var_unset_len_}"}}); do
...
##   引数チェックは行わない。
__sx_num_eq() {
	while SX_STR_EQ2({{"${2+X}"}}, {{X}}); do
		SX_STR_EQ2({{"$((${1} == ${2}))"}}, {{1}}) || return 1
		...
		case "${1}" in
			0[xX]* | -0[xX]*)
				if
					__sx_num_lt "${__sx_num_is_int_width_xlen_}" "${2}" || {
						SX_STR_EQ2({{"${__sx_num_is_int_width_xlen_}"}}, {{"${2}"}}) &&
			...
			*)
				if __sx_num_lt "${__sx_num_is_int_width_dlen_}" "${2}"; then
					unset __sx_num_is_int_width_arg_ __sx_num_is_int_width_bits_ __sx_num_is_int_width_dmax_ __sx_num_is_int_width_dmin_ __sx_num_is_int_width_dlen_ __sx_num_is_int_width_xlen_ __sx_num_is_int_width_olenn_ __sx_num_is_int_width_oleadn_ __sx_num_is_int_width_olenp_ __sx_num_is_int_width_oleadp_
					return 1
				elif SX_STR_EQ2({{"${__sx_num_is_int_width_dlen_}"}}, {{"${2}"}}); then
					# $1: 絶対値, $2: 制限値
					case "${1}" in
					...
					while :; do
						...
						if __sx_num_lt "${2}" "${1}"; then
							unset __sx_num_is_int_width_arg_ __sx_num_is_int_width_bits_ __sx_num_is_int_width_dmax_ __sx_num_is_int_width_dmin_ __sx_num_is_int_width_dlen_ __sx_num_is_int_width_xlen_ __sx_num_is_int_width_olenn_ __sx_num_is_int_width_oleadn_ __sx_num_is_int_width_olenp_ __sx_num_is_int_width_oleadp_
							return 1
						elif __sx_num_lt "${1}" "${2}" || SX_STR_EQ2({{"${3}"}}, {{''}}); then
							# 小さければ確定または残りがなければ終了
							break
						fi
...
sx_num_rel() {
	...
	SX_STR_EQ2({{"$((${#} == 0 ? 1 : ${#} % 2))"}}, {{1}}) || return "${SX_EX_USAGE}"

	__sx_num_rel_x=1
	for __sx_num_rel_arg in "${@}"; do
		if SX_STR_EQ2({{"${__sx_num_rel_x}"}}, {{1}}); then
...
__sx_num_rel() {
	...
	while SX_STR_EQ2({{"${2+X}"}}, {{X}}); do
		...
		SX_STR_EQ2({{"$(( __sx_num_rel_lhs_ ${__sx_num_rel_op_} ${2} ))"}}, {{1}}) || {
...
sx_str_any() {
	...
	for __sx_str_any_arg in "${@}"; do
		if SX_STR_EQ2({{"${__sx_str_any_tgt}"}}, {{"${__sx_str_any_arg}"}}); then
...
__sx_str_chunk() {
	...
	if __sx_num_lt 0 "${__sx_str_chunk_len_}"; then
		...
		while
			...
			! SX_STR_EQ2({{"${__sx_str_chunk_lim_}"}}, {{0}})
		do
		...
		if ! SX_STR_EQ2({{"${__sx_str_chunk_str_}"}}, {{''}}); then
...
__sx_str_rep() {
	...
	while ! SX_STR_EQ2({{"${3}"}}, {{0}}); do
		if SX_STR_EQ2({{"$((${3} % 2))"}}, {{1}}); then
...
__sx_str_split() {
	...
	if SX_STR_EQ2({{"${__sx_str_split_sep_}"}}, {{''}}); then
		if __sx_num_lt 0 "${__sx_str_split_lim_}"; then
			SX_STR_EQ2({{"${__sx_str_split_lim_}"}}, {{1}}) || __sx_str_chunk __sx_str_split_out_ "${__sx_str_split_str_}" 1 "$((__sx_str_split_lim_ - 1))"
	...
	if __sx_num_le 0 "${__sx_str_split_lim_}"; then
		while
			...
			! SX_STR_EQ2({{"${__sx_str_split_lim_}"}}, {{0}})
		do
...
__sx_str_sub() {
	...
	if SX_STR_EQ2({{"${__sx_str_sub_pat_}"}}, {{''}}); then
		...
		if __sx_num_lt 0 "${__sx_str_sub_lim_}"; then
			...
			while
				! SX_STR_EQ2({{"${__sx_str_sub_str_}"}}, {{''}}) &&
				! SX_STR_EQ2({{"${__sx_str_sub_lim_}"}}, {{1}})
			do
	...
	if __sx_num_le 0 "${__sx_str_sub_lim_}"; then
		...
		while
			...
			! SX_STR_EQ2({{"${__sx_str_sub_lim_}"}}, {{0}})
		do
...
sx_arr_pop() {
	...
	for __sx_arr_pop_dest in "${@}"; do
		...
		! SX_STR_EQ2({{"${__sx_arr_pop_dest}"}}, {{-}}) || continue
...
__sx_arr_pop0() {
	...
	for __sx_arr_pop0_dest_ in "${@}"; do
		...
		if ! SX_STR_EQ2({{"${__sx_arr_pop0_dest_}"}}, {{-}}); then
