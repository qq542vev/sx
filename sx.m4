...
##   パターンが空の場合は __sx_str_isep に委譲する。
__sx_str_sub_cb() {
	set -- "${1}" "${2-}" "${3-}" "${4-}" "${5-}" "$((${6-0} & SX_STR_SUB_GLOB))" "" 0 "" 0
	...
	if M_STR_EQ([|"${3}"|], [|''|]); then
		...
		do
			...
			set -- "${@}" "${2#"${11}"}"
			set -- "${1}" "${12}" "${3}" "${4}" "${5}" "${6}" "${7}${11}" "$((${8} + 1))" "${9}${11}" "${13%"${12}"}"

			"${4}" __sx_str_sub_cb_ "${10}" "${9}" "${2}" "${8}" || set -- "${@}" "${?}"

			set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}${__sx_str_sub_cb_-${10}}" "$((0 < ${11-0} ? ${5} : ${8}))" "${9}${10}" "${11-0}"
		...
		do
			...
			set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${__sx_str_sub_cb_-${10}}${7}" "$((0 < ${11-0} ? ${5} : ${8}))" "${10}${9}" "${11-0}"
			unset __sx_str_sub_cb_
		done
