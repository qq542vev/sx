...
##      ネイティブ除算で確定する。
##   6) 高速パス 5: 除数が WLEN-2 桁以内なら語幅 m = WLEN - len(v)（>= 2）の
...
__sx_num_int_divmod_abs() {
	...
	# ステップ 6: 高速パス 5 — 除数が WLEN-2 桁以内なら語単位のネイティブ筆算
	#   語幅を m = WLEN - len(v) へ拡大しており、m >= 2 が常に保証される。
	#   （m = 1 は商文字列の連結コストが割高になり Knuth D 法が優位のため対象外とする）
	case "$(( ${#__sx_num_int_divmod_abs_v_} < __sx_num_int_divmod_abs_wlen_ - 1 ))" in
		1)
