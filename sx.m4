...
##   コールバックのシグネチャ: callback 結果変数名 intrval prev next count
##     intrval: インターバル値 (符号付き整数)
##     prev: 挿入箇所より前方の文字列 (Prefix)
##     next: 挿入箇所より後方の文字列 (Suffix)
##     count: マッチカウンター
...
__sx_str_isep() {
	...
	# ${9}: count (マッチカウンター)
	# ${10}: ctx (既処理部分)
	# ${11}: orig_int (元の符号付きインターバル)
	# --- マッチごとの一時変数 ---
	# ${12}: next_str, ${13}: chunk, ${14}: ctx_updated, ${15}: stat
	set -- "${1}" "${2-}" "${3-}" "${4:-1}" "${5:-${SX_NUM_I32_MAX}}" "${6:-0}"

	# インターバルの絶対値が文字列長以上ならば挿入箇所がないため早期終了
	# (0 < ${4} ? 0 - ${4} : ${4}) は常に負の絶対値。これを -${#2} と比較することで INT_MIN を安全に扱う。
	case "$(((0 < ${4} ? 0 - ${4} : ${4}) <= 0 - ${#2}))" in 1)
		__sx_var_set "${1}=${2}"
		return "${SX_EX_OK}"
	esac
	...
	set -- "${1}" "${2}" "${3}" "${4}" "${5}" "$((${6} & SX_STR_ISEP_CB))" "" "${__sx_str_isep_qm_}" 0 "" "${4}"
	unset __sx_str_isep_qm_

	if M_NUM_LT([|0|], [|${4}|]); then
		# Forward (正方向)
		while
			M_NUM_LT([|${4}|], [|${#2}|]) && M_NUM_LT([|${9}|], [|${5}|])
		do
			set -- "${@}" "${2#${8}}"
			set -- "${1}" "${12}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}" "$((${9} + 1))" "${10}" "${11}" "${2%"${12}"}"

			# $2: next_str, $12: chunk
			case "${6}" in
				0) set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}${12}${3}" "${8}" "${9}" "${10}${12}" "${11}";;
				*)
					"${3}" __sx_str_isep_cb_ "${11}" "${10}${12}" "${2}" "${9}" || set -- "${@}" "${?}"
					set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}${12}${__sx_str_isep_cb_-}" "${8}" "$((0 < ${13-0} ? ${5} : ${9}))" "${10}${12}" "${11}"
		...
		do
			...
			# $2: prev_str, $12: chunk
			case "${6}" in
				0) set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${3}${12}${7}" "${8}" "${9}" "${12}${10}" "${11}";;
				*)
					"${3}" __sx_str_isep_cb_ "${11}" "${2}" "${12}${10}" "${9}" || set -- "${@}" "${?}"
					set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${__sx_str_isep_cb_-}${12}${7}" "${8}" "$((0 < ${13-0} ? ${5} : ${9}))" "${12}${10}" "${11}"
					unset __sx_str_isep_cb_
					;;
			esac
		done
...
##     関数名 結果変数名 マッチ文字列 前文脈(未置換) 後文脈(未置換) マッチ回数(1〜)
...
__sx_str_sub() {
	...
	# ${10}: ctx (未置換の文脈バッファ)
	# --- 以下、マッチごとの一時変数 ---
	# ${11}: pre/suf_rel, ${12}: suf/match, ${13}: stat
	...
	if M_STR_EQ([|"${3}"|], [|''|]); then
		...
		do
			...
			case "${7}" in
				...
				*)
				# コールバック置換: match:$11, prev:$10, next:$2, count:$9
					"${4}" __sx_str_sub_cb_ "${11}" "${10}" "${2}" "${9}" || set "${@}" "${?}"

					set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${8}${__sx_str_sub_cb_-${11}}" "$((0 < ${12-0} ? ${5} : ${9}))" "${10}${11}"
		...
		do
			...
			case "${7}" in
				...
				*)
					# コールバック置換: match:$11, prev:$2, next:$10, count:$9
					"${4}" __sx_str_sub_cb_ "${11}" "${2}" "${10}" "${9}" || set -- "${@}" "${?}"

					set -- "${1}" "${2}" "${3}" "${4}" "${5}" "${6}" "${7}" "${__sx_str_sub_cb_-${11}}${8}" "$((0 < ${12-0} ? ${5} : ${9}))" "${11}${10}"
					unset __sx_str_sub_cb_
					;;
			esac
		done
