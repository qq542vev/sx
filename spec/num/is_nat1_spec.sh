Describe 'sx_num_is_nat1'
  ...
  Context '有効な入力'
    ...
    It '複数の有効な正の整数に対して成功を返すこと'
      When call sx_num_is_nat1 "1" "123" "456789"
      The status should be success
    End

    It '非常に大きな数値に対して成功を返すこと'
      When call sx_num_is_nat1 "9223372036854775807"
      The status should be success
    End
  End
  ...
  Context '無効な入力 - ゼロ'
    It 'ゼロに対して失敗を返すこと'
      When call sx_num_is_nat1 "0"
      The status should be failure
    End

    It '複数のゼロを持つ数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "0" "00"
      The status should be failure
    End

    It '有効な数値とゼロの組み合わせに対して失敗を返すこと'
      When call sx_num_is_nat1 "123" "0"
      The status should be failure
    End
  End
  ...
  Context '無効な入力 - 先頭ゼロ'
    It '先頭に0がある数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "01"
      The status should be failure
    End

    It '複数の先頭ゼロを持つ数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "001" "000"
      The status should be failure
    End

    It '有効な数値と先頭ゼロを持つ数値の組み合わせに対して失敗を返すこと'
      When call sx_num_is_nat1 "123" "045"
      The status should be failure
    End
  End
  ...
  Context '無効な入力 - 非数字文字'
    It '英字を含む数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "123a"
      The status should be failure
    End

    It '記号を含む数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "123@"
      The status should be failure
    End

    It '空白を含む数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "12 3"
      The status should be failure
    End

    It '複数の非数字文字を含む数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "a1b2c3"
      The status should be failure
    End
  End
  ...
  Context '無効な入力 - 負の数値'
    ...
    It '複数の負の数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "-1" "-2" "-3"
      The status should be failure
    End
  End
  ...
  Context '無効な入力 - 浮動小数点数'
    It '小数点を含む数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "1.23"
      The status should be failure
    End

    It '指数表記の数値に対して失敗を返すこと'
      When call sx_num_is_nat1 "1e3"
      The status should be failure
    End
  End
  ...
  Context '複合的な無効な入力'
    It '有効な数値と無効な数値の組み合わせに対して失敗を返すこと'
      When call sx_num_is_nat1 "123" "45a" "789"
      The status should be failure
    End

    It 'すべての引数が無効な場合に対して失敗を返すこと'
      When call sx_num_is_nat1 "abc" "12.3" "-456"
      The status should be failure
    End
  End
  ...
  Context '境界値テスト'
    ...
    It '最大値より1大きい数値に対して成功を返すこと'
      When call sx_num_is_nat1 "9223372036854775808"
      The status should be success
    End
  End
End
