...
Describe 'sx_glob_bracket'
	...
	Describe '正常系: ] の処理（先頭配置）'
		It '] 単体を処理する'
			When call sx_glob_bracket result ']'
		...
		It '] が中間にある場合も先頭に移動される'
			When call sx_glob_bracket result 'a]b'
	...
	Describe '正常系: 特殊文字の複合'
		...
		It '[ はブラケット式内でリテラルとして扱われる'
			When call sx_glob_bracket result ']x['
		...
		It 'すべての特殊文字を含む複合パターン'
			When call sx_glob_bracket result ']x!=.:y-'
	...
	Describe '正常系: \ の処理（エスケープ防止）'
		...
		It '通常文字と \ の混合'
			When call sx_glob_bracket result 'a\b\c'
			The variable result should equal '[abc\\]'
		End
	End
	...
	Describe '異常系'
		It '読み取り専用変数に対して EX_NOPERM を返す'
			...
			When call sx_glob_bracket MYRO_GS 'a'
