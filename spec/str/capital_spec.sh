...
Describe 'sx_str_capital()'
    ...
    It 'SENTフラグ：先頭に空白があっても最初のアルファベットを大文字化する'
        When call sx_str_capital res "  aaa" "${SX_STR_CAPITAL_SENT}"
        The variable res should equal "  Aaa"
    End

    It 'SENTフラグ：数字で始まる場合でも最初のアルファベットを大文字化する'
        When call sx_str_capital res "123abc" "${SX_STR_CAPITAL_SENT}"
        The variable res should equal "123Abc"
    End

    It 'KEEPフラグ：最初のアルファベットのみ大文字化し、他は維持する'
        When call sx_str_capital res "hello WORLD" "${SX_STR_CAPITAL_KEEP}"
        The variable res should equal "Hello WORLD"
    End
    ...
    It 'KEEPフラグ：文頭が空白の場合、何もしない（SENT無効時）'
        When call sx_str_capital res "  aaa" "${SX_STR_CAPITAL_KEEP}"
        The variable res should equal "  aaa"
    End
    ...
    It 'KEEP+SENT フラグ：先頭空白の場合、最初のアルファベットのみ大文字化'
        When call sx_str_capital res "  hello" "$((SX_STR_CAPITAL_KEEP | SX_STR_CAPITAL_SENT))"
        The variable res should equal "  Hello"
    End

    It 'KEEP+SENT フラグ：数字で始まる場合も最初のアルファベットのみ大文字化'
        When call sx_str_capital res "123abc" "$((SX_STR_CAPITAL_KEEP | SX_STR_CAPITAL_SENT))"
        The variable res should equal "123Abc"
    End

    It 'KEEP+SENT フラグ：混在ケースで最初のアルファベットのみ大文字化'
        When call sx_str_capital res "hello WORLD" "$((SX_STR_CAPITAL_KEEP | SX_STR_CAPITAL_SENT))"
        The variable res should equal "Hello WORLD"
    End
    ...
    It 'SENT フラグ：句読点で始まる場合も最初のアルファベットを大文字化する'
        When call sx_str_capital res "!!!hello" "${SX_STR_CAPITAL_SENT}"
        The variable res should equal "!!!Hello"
    End
    ...
    It 'SENT フラグ：単一文字を大文字化する'
        When call sx_str_capital res "a" "${SX_STR_CAPITAL_SENT}"
        The variable res should equal "A"
    End

    It 'KEEP フラグ：全大文字の場合は何もしない'
        When call sx_str_capital res "HELLO" "${SX_STR_CAPITAL_KEEP}"
        The variable res should equal "HELLO"
    End
    ...
    It 'KEEP フラグ：先頭が非アルファベット小文字で始まる場合は何もしない（最適化パス）'
        When call sx_str_capital res "1hello" "${SX_STR_CAPITAL_KEEP}"
