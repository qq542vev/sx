...
# 1. 配列名のみ（現在の長さまでチェック）
echo "Testing sx_arr_is_writable (name only)..."
arr_len=2
arr_0=val0
arr_1=val1
if sx_arr_is_writable arr; then
    echo "  arr (len=2): OK"
else
...
# 2. 複数範囲指定 (開始, 個数)
echo "Testing sx_arr_is_writable (multiple ranges)..."
# テスト用に別の名前を使用
arr2_len=10
# 0から6要素(0-5), 8から3要素(8-10)
if sx_arr_is_writable arr2 0 6 8 3; then
...
# 4. 個数省略のテスト
echo "Testing sx_arr_is_writable (count omitted)..."
arr3_len=5
if sx_arr_is_writable arr3 2; then
